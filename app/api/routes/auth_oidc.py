...
from app.api.helpers.utils import _secure_cookie
...
def _redirect_uri(request: Request) -> str:
    return str(os.getenv("HEXSHARE_PUBLIC_URL", "http://localhost:8099")).rstrip("/") + "/api/v1/auth/callback"
...
def build_router() -> APIRouter:
    ...
    async def login(
            request: Request,
            next: str = "/api/v1/user/dashboard",
            idp: str | None = None
    ):
    ...
    async def auth_signup(request: Request, next: str = "/api/v1/user/dashboard", idp: str | None = None):
        """
        Starts signup:
